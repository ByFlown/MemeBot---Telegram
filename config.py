...
MAX_POSITION_SIZE_SOL = float(
    os.getenv("MAX_POSITION_SIZE_SOL", "1.0")
)  # Max SOL per trade
MAX_TOTAL_PORTFOLIO_SOL = float(
    os.getenv("MAX_TOTAL_PORTFOLIO_SOL", "10.0")
)  # Max total portfolio
...
PERFORMANCE_UPDATE_INTERVAL_MINUTES = int(
    os.getenv("PERFORMANCE_UPDATE_INTERVAL_MINUTES", "15")
)

# Safety Configuration
EMERGENCY_STOP_LOSS_PERCENT = float(
    os.getenv("EMERGENCY_STOP_LOSS_PERCENT", "50.0")
...
PAPER_TRADING_INITIAL_BALANCE = float(
    os.getenv("PAPER_TRADING_INITIAL_BALANCE", "100.0")
)  # Starting SOL for paper trading
PAPER_TRADING_MAX_TRADE_PCT = float(
    os.getenv("PAPER_TRADING_MAX_TRADE_PCT", "0.10")
)  # Max 10% per trade
...
if not os.getenv("FLY_APP_NAME"):  # Only print locally, not on Fly.io
    ...
    print(
        f"   - Telegram Token: {'✅ Set' if TELEGRAM_TOKEN != 'DEIN_TELEGRAM_BOT_TOKEN' else '❌ Not set'}"
    )
    print(f"   - Owner ID: {OWNER_ID}")
    print(
        f"   - Real Trading: {'🚨 ENABLED' if REAL_TRADING_ENABLED else '📝 Paper only'}"
    )
    print(
        f"   - Wallet: {'✅ Configured' if SOLANA_PRIVATE_KEY else '❌ Not configured'}"
    )
