...
class SelfLearningTrader:
    ...
    def __init__(self, wallet_manager=None):
        ...
        self.model = SGDRegressor(
            random_state=42
        )  # Start with basic model for immediate predictions
        self.scaler = StandardScaler()
        self.online_model = SGDRegressor(random_state=42)
        self.is_trained = (
            False  # Will become True after first few trades provide training data
        )
        ...
        self.active_positions = (
            {}
        )  # {token_address: {entry_data, features, ml_prediction}}
    ...
    def extract_features(self, token_data: Dict) -> Dict:
        ...
        try:
            ...
            features["has_social_links"] = int(
                bool(token_data.get("website") or token_data.get("twitter"))
            )
            ...
            if liquidity > 0 and age_hours > 0:
                features["momentum_score"] = min(
                    10.0, (volume_24h / liquidity) * (24 / age_hours)
                )
            else:
                features["momentum_score"] = 0.0

            # Market Status Features
            features["is_boosted"] = int(
                token_data.get("boosted", False)
                or token_data.get("boosts", {}).get("active", False)
            )
            features["is_trending"] = int(
                token_data.get("trending", False)
                or token_data.get("trendingOn", []) != []
            )
    ...
    async def open_position(
        self, token_data: Dict, ml_confidence: float, position_size: float = None
    ) -> bool:
        """
        ...
        try:
            ...
            # Save to database
            self.save_position_entry(
                token_address, features, ml_confidence, position_size
            )
    ...
    def should_exit_position(
        self, profit_pct: float, position_age_hours: float, ml_confidence: float
    ) -> tuple[bool, str]:
        """
        ...
        if (
            ml_confidence > 0.8 and profit_pct > 0.15
        ):  # 15%+ profit with high confidence
            return True, "profit_target_high_confidence"
        elif (
            ml_confidence > 0.6 and profit_pct > 0.10
        ):  # 10%+ profit with medium confidence
            return True, "profit_target_medium_confidence"
        elif (
            ml_confidence <= 0.6 and profit_pct > 0.05
        ):  # 5%+ profit with low confidence
    ...
    async def close_position(
        self, token_address: str, exit_price: float, exit_reason: str
    ):
        """
        ...
        try:
            ...
            if self.wallet_manager:
                logger.info(
                    f"Executing sell trade for {token_symbol} ({token_address[:8]}...)"
                )
                ...
                if not trade_result.get("success", False):
                    logger.error(
                        f"Failed to execute sell trade for {token_symbol}: {trade_result.get('error', 'Unknown error')}"
                    )
                    # Continue with position tracking even if trade fails
                else:
                    logger.info(
                        f"✅ Successfully sold {token_symbol} - Trade result: {trade_result}"
                    )
            else:
                logger.warning(
                    "No wallet manager available - position closed in tracking only"
                )
            ...
            reward = self.calculate_profit_reward(
                profit_pct, profit_sol, holding_duration, ml_confidence, exit_reason
            )
            ...
            await self.update_model_with_profit(
                position["features"], profit_pct, reward
            )
    ...
    async def update_model_with_profit(
        self, features: Dict, profit_pct: float, reward: float
    ):
        """
    ...
    def update_position_with_results(
        self,
        token_address: str,
        exit_price: float,
        profit_pct: float,
    ...
    async def check_retrain_trigger(self):
        ...
        try:
            ...
            if completed_count > 0 and completed_count % 25 == 0:
                logger.info(
                    f"Triggering model retraining with {completed_count} completed trades"
                )
    ...
    def get_completed_trades_count(self) -> int:
        ...
        try:
            ...
            cursor.execute(
                "SELECT COUNT(*) FROM training_data WHERE position_closed = 1"
            )
    ...
    async def train_model(self, use_initial_split=False):
        ...
        try:
            ...
            ml_logger.model_update(
                ...
                update_type=(
                    "full_retrain" if use_initial_split else "incremental_retrain"
                ),
    ...
    def load_training_data(self) -> pd.DataFrame:
        ...
        try:
            ...
            df = pd.read_sql_query(
                "SELECT * FROM training_data WHERE position_closed = 1", conn
            )
            conn.close()
            return df
        ...
        try:
            ...
            models = {
                ...
                "gradient_boosting": GradientBoostingRegressor(
                    random_state=42, learning_rate=0.1, max_depth=6
                ),
            }
    ...
    async def evaluate_trade_opportunity(self, token_data: Dict) -> Dict:
        ...
        try:
            ...
            if not self.is_trained:
                ...
                should_trade = (
                    confidence >= self.trade_confidence_threshold
                ...
                should_trade = (
                    confidence >= self.trade_confidence_threshold
                    and expected_profit > self.min_profit_threshold
                )
    ...
    def _calculate_prediction_confidence(self) -> float:
        ...
        try:
            ...
            profitable_trades = sum(
                1 for profit, reward, conf in recent_trades if profit > 0
            )
            avg_reward = (
                sum(reward for profit, reward, conf in recent_trades) / total_trades
            )

            # Base confidence on win rate and reward score
            win_rate = profitable_trades / total_trades
            reward_factor = max(
                0, min(1, (avg_reward + 1) / 2)
            ...
            if main_model_path.exists() and scaler_path.exists():
                ...
                if metadata_path.exists():
                    with open(metadata_path, "r") as f:
                        ...
                        self.learning_window_minutes = metadata.get(
                            "learning_window_minutes", 20
                        )
                        self.profit_threshold_good = metadata.get(
                            "profit_threshold_good", 0.10
                        )
                        self.loss_threshold_bad = metadata.get(
                            "loss_threshold_bad", -0.10
                        )
    ...
    def get_training_stats(self) -> Dict:
        ...
        try:
            ...
            cursor.execute(
                "SELECT COUNT(*) FROM training_data WHERE position_closed = 0"
            )
            ...
            # Calculate win rate
            win_rate = (
                (profitable_trades / recent_trades * 100) if recent_trades > 0 else 0
            )
            ...
            if self.completed_trades:
                ...
                trading_history_stats = {
                    ...
                    "avg_recent_profit": sum(t["profit_pct"] for t in recent_trades)
                    / len(recent_trades),
                    "avg_recent_reward": sum(t["reward"] for t in recent_trades)
                    / len(recent_trades),
                    "avg_recent_duration": sum(
                        t["holding_duration"] for t in recent_trades
                    )
                    / len(recent_trades),
                }
            ...
            if self.is_trained:
                model_performance = {
                    "model_type": (
                        type(self.model).__name__ if self.model else "Unknown"
                    ),
            ...
            return {
                ...
                "boosted_win_rate": (
                    (profitable_boosted / total_boosted * 100)
                    if total_boosted > 0
                    else 0
                ),
                ...
                "trending_win_rate": (
                    (profitable_trending / total_trending * 100)
                    if total_trending > 0
                    else 0
                ),
    ...
    def get_ml_accuracy_metrics(self) -> Dict:
        ...
        try:
            ...
            predicted_profitable = [
                conf > 0.5 for conf in confidences
            ]  # Above 50% confidence = predicted profit
            actual_profitable = profitable_trades

            direction_correct = sum(
                1
                for pred, actual in zip(predicted_profitable, actual_profitable)
                if pred == actual
            )
            ...
            high_conf_trades = [
                (conf, profit)
                for conf, profit in zip(confidences, profitable_trades)
                if conf > 0.7
            ]
            low_conf_trades = [
                (conf, profit)
            ...
            if len(confidences) > 1:
                ...
                avg_conf_profitable = sum(
                    conf
                    for conf, profit in zip(confidences, profitable_trades)
                    if profit
                ) / max(sum(profitable_trades), 1)
                avg_conf_unprofitable = sum(
                    conf
                    for conf, profit in zip(confidences, profitable_trades)
                    if not profit
                ) / max(len(profitable_trades) - sum(profitable_trades), 1)
            ...
            predicted_profits = [
                (conf - 0.5) * 0.4 for conf in confidences
            ]  # Scale confidence to expected profit range
            actual_profits = profit_percentages

            squared_errors = [
                (pred - actual) ** 2
                for pred, actual in zip(predicted_profits, actual_profits)
            ]
            profit_prediction_rmse = (
            ...
            model_performance_score = (
                self.get_model_performance() * 100 if self.is_trained else 0.0
            )
            ...
            return {
                ...
                * 100,  # How well confidence correlates with success
    ...
    async def _generate_initial_training_data(self):
        ...
        try:
            ...
            logger.info(
                "🔥 Generating immediate training data for instant ML training..."
            )
            ml_logger.analysis(
            ...
            samples_generated = await generator.generate_immediate_training_data(
                batch_size=300
            )
            ...
            if samples_generated > 0:
                logger.info(
                    f"✅ Generated {samples_generated} training samples from real token data"
                )
                ...
                if (
                    labeled_count >= 20
                ):  # Lower threshold since we're using real data only
                    logger.info(
                        "🧠 Performing initial 80/20 training with backtesting data..."
                    )
                    ...
                    if training_results:
                        ...
                        logger.warning(
                            "⚠️ Initial training completed but no results returned"
                        )
                else:
                    logger.info(
                        f"⏳ Only {labeled_count} real samples generated, need 20+ for training. Will continue collecting live data."
                    )
            else:
                logger.warning(
                    "⚠️ Could not fetch real token data from DexScreener - will collect data through live trading"
                )
